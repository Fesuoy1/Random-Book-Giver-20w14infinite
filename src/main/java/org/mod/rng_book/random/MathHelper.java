...
public class MathHelper {
    ...
    public static boolean method_34945(Vec3d origin, Vec3d direction, Box box) {
        double d = (box.getMin(Axis.X) + box.getMax(Axis.X)) * 0.5;
        double e = (box.getMax(Axis.X) - box.getMin(Axis.X)) * 0.5;
        double f = origin.x - d;
        if (Math.abs(f) > e && f * direction.x >= 0.0) {
            return false;
        }
        double g = (box.getMin(Axis.Y) + box.getMax(Axis.Y)) * 0.5;
        double h = (box.getMax(Axis.Y) - box.getMin(Axis.Y)) * 0.5;
        double i = origin.y - g;
        if (Math.abs(i) > h && i * direction.y >= 0.0) {
            return false;
        }
        double j = (box.getMin(Axis.Z) + box.getMax(Axis.Z)) * 0.5;
        double k = (box.getMax(Axis.Z) - box.getMin(Axis.Z)) * 0.5;
        double l = origin.z - j;
    ...
    public static float inverseSqrt(float x) {
        return (float) MathHelper.fastInverseSqrt(x);
    }

    public static double inverseSqrt(double x) {
        return MathHelper.fastInverseSqrt(x);
    }
...
public static int hsvToRgb(float hue, float saturation, float value) {
    ...
    int red = MathHelper.clamp((int)(k * 255.0f), 0, 255);
    int green = MathHelper.clamp((int)(l * 255.0f), 0, 255);
    int blue = MathHelper.clamp((int)(result * 255.0f), 0, 255);
...
public static IntStream stream(int seed, int lowerBound, int upperBound, int steps) {
    ...
    return IntStream.iterate(seed, i -> {
        ...
        return seed - m >= lowerBound || seed + m <= upperBound ? seed - m : seed + m + steps;
    });
}
